...
function draw() {
    if ( frameCount == 1) {
        ...
        translate(0, height);
        scale(1, -1);
        //circle(bolt_tails[0][0], height + -1*bolt_tails[0][1], 51);
        //circle(bolt_tails[1][0], height + -1*bolt_tails[1][1], 51);
        //circle(bolt_tails[2][0], height + -1*bolt_tails[2][1], 51);
        var tx = 0;
        var ty = 0;
        for (var lol = 0; lol < 13; lol ++) {
            ...
            for (var bdx = 0; bdx < bolt_tails.length; bdx++) {
                ...
                var e2 = eb - tail[1];
                var displ = 50;
                if (dist(ea, eb, bolt_tails[bdx][0], height + -1 * bolt_tails[bdx][1]) < 100
                || dist(ea + displ, eb, bolt_tails[bdx][0] + displ, height + -1 * bolt_tails[bdx][1]) < 100
                || dist(ea - displ, eb, bolt_tails[bdx][0] - displ, height + -1 * bolt_tails[bdx][1])< 100) 
                {
                    tree(tx, ty, 0, 3);
                } 
    ...
    for (var i=0; i<100; i++) {
        ...
        for (var ldx = 0; ldx < bolts.length - 1; ldx++) { 
            ...
            if (i == 99 && ldx == num_bolts - 1) {
                strike.push(bolts[ldx][0]);
                strike.push(bolts[ldx][1]);
